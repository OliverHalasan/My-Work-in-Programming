...
namespace WebApp.Pages.ExercisePages
{
    public class RecordPickingModel : PageModel
    {
        ...
        public IActionResult OnPostSave()
        {
            ...
            try
            {
                if (OrderId.HasValue || PickerId.HasValue)
                {
                    _pickingservices.Picking_RecordPicking((int)OrderId, (int)PickerId, PickedItems);
                    FeedBackMessage = "Order has been saved";
                } else
                {
                    FeedBackMessage = "find an order to maintain before attempting the update";
                }
